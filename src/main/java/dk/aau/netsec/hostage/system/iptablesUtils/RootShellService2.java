...
import java.util.Iterator; // Changed from ListIterator
...
import eu.chainfire.libsuperuser.Shell.OnShellOpenResultListener;
...
public class RootShellService2 extends Service {
    ...
    private static void processCommands(final RootCommand state) {
        if (state.commandIndex < state.getCommmands().size() && state.getCommmands().get(state.commandIndex) != null) {
            ...
            if (command != null) {
                ...
                try {
                    rootSession.addCommand(command, 0, new OnCommandResultListener() {
                        @Override
                        public void onCommandResult(int commandCode, int exitCode, List<String> output) {
                            if (output != null) {
                                Iterator<String> iter = output.iterator(); // use iterator()
                                while (iter.hasNext()) {
                            ...
                            if (exitCode >= 0 && exitCode == state.retryExitCode && state.retryCount < MAX_RETRIES) {
                                state.retryCount++;
                                Log.d(TAG, "command '" + state.lastCommand + "' exited with status "
                                        + exitCode + ", retrying (attempt " + state.retryCount + "/" + MAX_RETRIES + ")");
                                processCommands(state);
                                return;
                            }
                            ...
                            if (state.commandIndex >= state.getCommmands().size() || errorExit) {
                                ...
                                if (exitCode < 0) {
                                    ...
                                    Log.e(TAG, "libsuperuser error " + exitCode + " on command '"
                                            + state.lastCommand + "'");
                                } else {
                                    if (errorExit) {
    ...
    private void setupLogging() {
        ...
        Debug.setOnLogListener((type, typeIndicator, message) ->
    ...
    private void startShellInBackground() {
        ...
        if (rootSession == null) {
            rootSession = new Shell.Builder()
                    ...
                    .open(new OnShellOpenResultListener() { // Changed signature
                        @Override
                        public void onOpenResult(boolean succeeded, int reason) {
                            if (!succeeded) {
                                Log.e(TAG, "Can't open root shell: reason " + reason);
                                rootState = ShellState.FAIL;
                            } else {
    ...
    public void runScriptAsRoot(Context ctx, List<String> cmds, RootCommand state) {
        ...
        if (rootState == ShellState.INIT ||
                (rootState == ShellState.FAIL && state.reopenShell)) {
            reOpenShell(ctx);
