...
import dk.aau.netsec.hostage.protocol.COAP;
...
public class COAPListener extends Listener {
     ...
     */
    public COAPListener(Hostage service, Protocol protocol) {
        super(service, protocol);
    }

    public COAPListener(Hostage service, Protocol protocol, int port) {
    ...
    private Thread serverThread(){
        serverThread =  new Thread(() -> {
            try {
                ...
                if(COAPHandler.isAnAttackOngoing()) {
                    startHandler();
